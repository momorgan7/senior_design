...
(function()
   ...
   var defaultSnapshotAspectRatio = 4 / 3;
   ...
   org.gigapan.snapshot.SnapshotTool = function(viewer, desiredSnapshotAspectRatio)
      {
      ...
      var willConstrainSnapshotAspectRatio = (desiredSnapshotAspectRatio !== undefined);
      var snapshotAspectRatio = (willConstrainSnapshotAspectRatio) ? Math.abs(desiredSnapshotAspectRatio) : defaultSnapshotAspectRatio;
      ...
      jQuery.each(snapshotToolShadedAreas,
	    function(i)
               {
	       snapshotToolShadedAreas[i] = jQuery('<div class="snapshot_tool_shaded_area"><div></div></div>')[0];
               });
      ...
      /**
       * Sets the snapshot tool's bounds to the given Rect (in Gigapan coordinates)
       */
      this.setToolBoundsInGigapanCoords = function(boundsRectInPointsCoords)
         {
         if (!this.isVisible)
            {
      ...
      this.setVisible = function(willMakeVisible)
         ...
         if (willMakeVisible && !isSnapshotToolVisible)
            ...
            // a 4x3 aspect ratio and is centered in the visible area.
            ...
            if (bottomRightOfVisiblePortionInPixelCoordinates.x > topLeftOfVisiblePortionInPixelCoordinates.x &&
               ...
               // set the dimensions so it's 4x3 and 90% of the shorter dimension
               if (intersectingRectInPixelCoords.width > intersectingRectInPixelCoords.height)
                  {
   ...
   function DraggableWidgetMouseTracker(element, viewer)
      ...
      this.getCenterPointInPixelCoords = function(element)
         {
         var elementHalfDimensions = Seadragon.Utils.getElementSize(element).divide(2);

         var topLeftCorner = Seadragon.Utils.getElementPosition(element).minus(Seadragon.Utils.getElementPosition(this.viewer.elmt));
